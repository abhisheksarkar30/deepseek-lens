...
| `docs/context/api-surface.md`, `docs/context/data-model.md` | Refresh in Phase 5.6: `by_day`/`DayStat`/`StatsByDay` references become `by_period`/`PeriodStat`/`StatsByPeriod`, plus the new `granularity`/`until` params; recompute the drifted line citations in the `api-surface.md` route table — *after* the implementation edits land, not hard-coded now, since `stats()` will grow further during this very change. This plan estimated four rows off by a uniform ~72-line offset (it named the `/api/stats`, `/api/warnings/summary`, `/api/warnings` and `/api/sessions` rows, then cited at `api.go:652-700`, `701-718`, `719-754`, `755-803`), and that estimate was wrong on both the count and the uniformity: the real class is twenty-one citations across the file, off by +59 to +140. The ten route-table rows citing a *handler* span +85 to +124; the catch-all `/` row cites the registration line rather than a handler and moved +61; the outliers are the `mux.Handle*` block citation at +59 and the `healthResponse` payload citation at +140. The spread exists because the edits that shifted them move only the rows below them. All twenty-one were recomputed from the edited file by brace-matching each citation's handler or region, and each was verified to bracket a known anchor line. The citations are all in one already-open file, so fixing the class cost nothing extra over fixing the four (F6.4) |
...
| R5 | Context docs (`api-surface.md`, `data-model.md`) go stale the moment this lands | Phase 5.6 refresh scoped to this rename + the new `granularity`/`until` params + recomputing (post-edit, not pre-coded) the drifted line citations in `api-surface.md`. This plan estimated four citations off by a uniform offset; the real class proved to be twenty-one, off by +59 to +140, and all were recomputed from the edited file (see the Phase 5.6 What-changes row) |
