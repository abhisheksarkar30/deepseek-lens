...
| **F3** | `export ANTHROPIC_BASE_URL=http://127.0.0.1:8787` had **no effect** on this Claude Code installation: the first CLI call never reached lens (`sink_accepted` stayed 0), and the prompt was answered by the upstream directly. | Claude Code's `~/.claude/settings.json` carries an `env` block that takes precedence over the process environment. Pointing an existing install at the proxy needs `claude --settings <file>` (as used above) or an edit to `settings.json`. | **Open, documentation-level.** The README's quick start says `export` then `claude`; for an install with `ANTHROPIC_BASE_URL` in `settings.json` that is not enough. Verified fix: the `--settings` form used in step 2a. |
