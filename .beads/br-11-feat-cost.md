# Bead 11: Token and cost accounting
...
Turn the token counts parsed in bead 4 into money, honestly labelled.
...
CLI prints `—` (bead 8's `humanCost` already does this for nil), the dashboard renders a `?` badge
...
`pricing.Compute(model string, usage parse.Usage, table Table) Cost` is pure and table-driven, with
integer/duration-safe arithmetic — it uses `math/big.Rat` internally for the per-token multiply so
that summing ten thousand calls cannot accumulate floating-point drift, then rounds half-up to
micro-dollars at the boundary. (`ponytail:` big.Rat for money; a decimal library is not warranted
for a single multiply-and-sum.)
...
to `"approximate"`. Given DeepSeek ignores `cache_control` entirely (bead 10), these fields will
normally be absent — the handling exists so the accounting is not silently wrong if that changes.

The consumer's analyzer hook attaches the computed cost to the row before insert; `requests.cost_usd`
and `requests.cost_source` are populated (columns already exist from bead 6).
...
Token counting already landed in bead 4; this bead is the part with a correctness trap. The trap is
...
- Unit Tests (`internal/pricing/pricing_test.go`):
  ...
  - **Drift**: 10,000 calls of 1 input token at $0.28/M → exact expected total, no float error.
  - **Rounding**: a sub-micro-dollar value rounds half-up, not truncates.
...
- `internal/consumer/consumer.go` (modify — register the cost analyzer)
